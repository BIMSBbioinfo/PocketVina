...
parser.add_argument('--box_length', type=int, default=26, help='Box length.')
...
def process_ligand(ligand: str, input_dir: str, base_output_dir: str):
    ...
    Process a single ligand:
      ...
      - Load the ligand molecule from an SDF file (named "{ligand}_ligands.sdf").
    ...
    sdf_input_file = os.path.join(input_dir, f"{ligand}/{ligand}_ligands.sdf")
...
def main_conversion(testset_file: str):
    ...
    sub_output_dir = f"{args.results_dir}/posebuster/{args.p2rank_model}_prank_thread{args.p2rank_threads}/{args.box_length}_Angst/run_{args.run_number}/results"
    base_output_dir = os.path.join(base_dir, sub_output_dir)
    os.makedirs(base_output_dir, exist_ok=True)
    
    for ligand in ligand_list:
        process_ligand(ligand, os.path.join(base_dir, "PROTEIN_DB/posebusters_benchmark_set"), base_output_dir)
...
def process_single_pdb_id(pdb_id: str) -> list:
    ...
    """
    results = []
    base_dir = f"/fast/AG_Akalin/asarigun/Arcas_Stage_1/ROOF/PROTEIN_DB/posebusters_benchmark_set/{pdb_id}"
    ref_sdf = os.path.join(base_dir, f"{pdb_id}_ligands.sdf")
...
def main_docking(testset_file: str):
    ...
    results = process_testset(testset_file, limit=429)
    common_base_dir = "/fast/AG_Akalin/asarigun/Arcas_Stage_1/ROOF"
    sub_output_dir = f"{args.results_dir}/posebuster/{args.p2rank_model}_prank_thread{args.p2rank_threads}/{args.box_length}_Angst/run_{args.run_number}/results"
...
def main():
    testset_file = "data/timesplit_test_posebusters_benchmark_set"  # File containing pdb_ids, one per line.
