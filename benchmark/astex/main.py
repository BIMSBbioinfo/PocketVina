...
parser.add_argument('--box_length', type=int, default=30, help='Box length.')
...
def main_conversion(testset_file: str):
    ...
    sub_output_dir = f"{args.results_dir}/astex/{args.p2rank_model}_prank_thread{args.p2rank_threads}/{args.box_length}_Angst/run_{args.run_number}/results"
    base_output_dir = os.path.join(base_dir, sub_output_dir)
    os.makedirs(base_output_dir, exist_ok=True)
    
    for ligand in ligand_list:
        process_ligand(ligand, os.path.join(base_dir, "PROTEIN_DB/astex_diverse_set"), base_output_dir)
...
def process_single_pdb_id(pdb_id: str) -> list:
    ...
    """
    results = []
    base_dir = f"/fast/AG_Akalin/asarigun/Arcas_Stage_1/ROOF/PROTEIN_DB/astex_diverse_set/{pdb_id}"
...
def main():
    testset_file = "data/timesplit_test_astex_diverse_set"  # File containing pdb_ids, one per line.
